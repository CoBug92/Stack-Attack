...
enum CargoVisualRenderer {
	...
	static func makeCrate(side: CGFloat, color: UIColor) -> SKNode {
		...
		ScenePrimitives.rect(
			...
			UIColor(resource: .Theme.white).withAlphaComponent(0.18),
			CGPoint(x: 0, y: 15), 4
		)
		ScenePrimitives.rect(
		...
		let steel = UIColor(resource: .Cargo.metal)
		...
		ScenePrimitives.rect(
			node, CGSize(width: 3, height: 15), UIColor(resource: .Theme.white).withAlphaComponent(0.6),
			CGPoint(x: -2, y: 0), 6
		)
		ScenePrimitives.rect(
			node, CGSize(width: 3, height: 15), UIColor(resource: .Theme.white).withAlphaComponent(0.35),
			CGPoint(x: 3, y: 0), 6
		)
		ScenePrimitives.rect(
			node, CGSize(width: 20, height: 4), UIColor(resource: .Cargo.strap).withAlphaComponent(0.8),
			CGPoint(x: 0, y: -15), 6
		)
		ScenePrimitives.rect(
			node, CGSize(width: 20, height: 4), UIColor(resource: .Cargo.strap).withAlphaComponent(0.68),
			CGPoint(x: 0, y: 15), 6
		)
		ScenePrimitives.roundedRect(
			node, CGSize(width: 18, height: 12), UIColor(resource: .Cargo.label),
			CGPoint(x: -10, y: 9), cornerRadius: 2, z: 7)
		for y in [13, 10, 7] as [CGFloat] {
			ScenePrimitives.rect(
				node, CGSize(width: 11, height: 1), UIColor(resource: .Cargo.labelLines),
				CGPoint(x: -10, y: y), 8
			)
		}
		ScenePrimitives.roundedRect(
			node, CGSize(width: 12, height: 8), UIColor(resource: .Cargo.stamp),
			CGPoint(x: 12, y: -10), cornerRadius: 2, z: 7)
		ScenePrimitives.rect(
			node, CGSize(width: 8, height: 2), UIColor(resource: .Cargo.stampHighlight),
			CGPoint(x: 12, y: -10), 8
		)
